...
use budgetchain_contracts::interfaces::IMilestoneManager::{IMilestoneManagerDispatcher, IMilestoneManagerDispatcherTrait};
...
// Helper function to deploy the MilestoneManager contract
fn deploy_milestone_manager(admin: ContractAddress) -> (ContractAddress, IMilestoneManagerDispatcher) {
...
fn test_create_milestone() {
    ...
    // Create milestone
    let milestone_id = dispatcher.create_milestone(
        org, project_id, milestone_description, milestone_amount
    );
...
fn test_create_multiple_milestones() {
    ...
    let milestone_id1 = dispatcher.create_milestone(
        org, project_id, milestone_description, milestone_amount
    );
    
    // Create second milestone
    let milestone_id2 = dispatcher.create_milestone(
        org, project_id, 'Second milestone', milestone_amount * 2
    );
    
    // Verify milestone IDs are correct
        ...
        @array![
            (
                ...
                MilestoneManager::Event::MilestoneCompleted(
                    MilestoneManager::MilestoneCompleted {
                        project_id,
                        milestone_id,
                    },
                ),
...
fn test_cannot_complete_milestone_twice() {
    ...
    // Create and complete milestone as admin
    cheat_caller_address(contract_address, admin, CheatSpan::TargetCalls(2));
    let milestone_id = dispatcher.create_milestone(
        org, project_id, milestone_description, milestone_amount
    );
    ...
    // Try to complete the milestone again (should fail)
    cheat_caller_address(contract_address, admin, CheatSpan::TargetCalls(1));
    dispatcher.set_milestone_complete(project_id, milestone_id);
}
...
fn test_cannot_complete_nonexistent_milestone() {
    ...
    // Try to complete a nonexistent milestone (should fail)
    cheat_caller_address(contract_address, admin, CheatSpan::TargetCalls(1));
...
fn test_unauthorized_cannot_create_milestone() {
    ...
    // Try to create milestone as non-org (should fail)
    cheat_caller_address(contract_address, non_org, CheatSpan::TargetCalls(1));
    dispatcher.create_milestone(
        org, project_id, milestone_description, milestone_amount
    );
}
...
fn test_pause_and_unpause_contract() {
    ...
    // Create milestone after unpausing (should succeed)
    cheat_caller_address(contract_address, admin, CheatSpan::TargetCalls(1));
    let milestone_id = dispatcher.create_milestone(
        org, project_id, milestone_description, milestone_amount
    );
...
fn test_cannot_create_milestone_when_paused() {
    ...
    // Try to create milestone while paused (should fail)
    cheat_caller_address(contract_address, admin, CheatSpan::TargetCalls(1));
    dispatcher.create_milestone(
        org, project_id, milestone_description, milestone_amount
    );
...
fn test_only_admin_can_unpause_contract() {
    ...
    // Try to unpause contract as non-admin (should fail)
